...
import { supabase, SUPABASE_URL_VAL } from '../services/supabase.ts';
...
const AuthModal: React.FC<AuthModalProps> = ({ onClose, initialEmail, initialStoreUrl, initialMode = 'login' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!SUPABASE_URL_VAL || SUPABASE_URL_VAL.includes('placeholder')) {
      setError("Compliance Vault Unreachable: Database credentials not detected in your Netlify Environment. Please check your setup.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(err.message || "An unexpected error occurred while accessing the vault.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
        ...
        <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl p-12 text-center animate-in zoom-in duration-300">
          ...
          <p className="text-slate-500 font-medium leading-relaxed mb-10 text-sm italic">
            Check <strong>{email}</strong> to verify your identity and activate your vault.
          </p>
      ...
      <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in duration-300">
        <div className="p-10 lg:p-12">
          <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-none uppercase italic mb-6">
            {mode === 'signup' ? 'New Vault' : mode === 'login' ? 'Merchant Login' : 'Recovery'}
          </h2>

          <form onSubmit={handleSubmit} className="space-y-5">
            {error && (
              <div className="p-4 bg-red-50 text-red-600 rounded-2xl text-[10px] font-black uppercase border border-red-100 italic leading-relaxed">
                {error}
              </div>
            )}
            ...
            <input required type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-transparent focus:bg-white focus:ring-2 focus:ring-emerald-500/20 outline-none font-bold text-sm" placeholder="Vault Access Key" />
            ...
            <button disabled={loading} type="submit" className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-xl disabled:opacity-50">
              {loading ? 'Processing Registry...' : 'Initiate Handshake'}
            </button>
          </form>
